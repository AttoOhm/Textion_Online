"""
Chop Command (Phase 7B)

Short command: chop <resource>
Chop trees for wood.

Requires:
- Hatchet equipped (main_hand or off_hand)
...
# Woodcutting resources - Primary and Secondary woods
WOODCUTTING_RESOURCES = [
    # Primary woods
    "oak_tree",
    "ash_tree",
    "yew_tree",
    # Secondary woods (resins/saps)
    "resin_deposit",
    "sap_deposit",
    "spirit_bark_deposit",
]
...
def handle_chop(player_state, resource_name, map_id, node_id,
    ...
    if not tool_found:
        return {
            "success": False,
            "message": f"You need a {REQUIRED_TOOL} equipped to chop trees.",
        }
    ...
    output_item = get_woodcutting_output(resource_name_lower)
    if not output_item:
        return {
            "success": False,
    ...
    if existing_action:
        print(f"[CHOP DEBUG] Found existing action: id={existing_action.get('action_id')}, status={existing_action.get('status')}, end_tick={existing_action.get('end_tick')}, current_tick={current_tick}")
    ...
    return {
        "success": True,
        "message": f"You begin chopping the {resource_name}...",
        "action_id": action["action_id"],
        "end_tick": action["end_tick"],
    }


def complete_chop(action, resource_node_manager, inventory_engine,
                  equipment_engine, current_tick=0):
    """Complete a woodcutting action.

    Args:
        action: Completed long action dict
        resource_node_manager: ResourceNodeManager instance
        inventory_engine: InventoryEngine instance or list (player_state['inventory'])
        equipment_engine: EquipmentEngine instance
        current_tick: Current world tick
    ...
    if hasattr(inventory_engine, 'add_item'):
        ...
        from engine.inventory import create_inventory_entry
        inventory_engine.append(create_inventory_entry(output_item, 1))
    ...
    if equipment_engine:
        ...
        if tool_entry:
            ...
            if new_durability <= 0:
                ...
                return {
                    "success": True,
                    "message": f"You chop the tree, but your {REQUIRED_TOOL} breaks!",
                    "tool_broken": True,
                }
    
    return {
        "success": True,
        "message": f"You chop the tree and obtain {get_item_name(output_item)}.",
        "tool_broken": False,
    }


def get_woodcutting_output(resource_type: str) -> str:
    """Get the output item for a woodcutting resource.

    Args:
        resource_type: Resource node type (e.g., "oak_tree")
    ...
    woodcutting_outputs = {
        ...
        "resin_deposit": "resin",
        "sap_deposit": "sap",
        "spirit_bark_deposit": "spirit_bark",
    }
    return woodcutting_outputs.get(resource_type, "")
