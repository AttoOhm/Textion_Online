"""
Bless Command (Divine Technique)

Real command: cast bless <target>

Blesses a target, enhancing their combat abilities.
...
from engine.combat_techniques import CombatTechniqueEngine, TECHNIQUE_BLESS


def handle_bless(player_state, phrase_input, world_map, current_tick,
                 action_queue, technique_manager=None, equipment_engine=None,
                 combat_engine=None):
    """Handle bless casting."""
    player_id = player_state.get("id", "player")

    if technique_manager and not technique_manager.has_technique(player_id, "bless"):
        return {
            "success": False,
            "message": "You do not know the Bless technique.",
            "events": []
        }

    # Find target (optional - can self-target)
