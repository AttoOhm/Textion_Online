"""
Resurrection Command (Divine Technique)
Real command: cast resurrection <target>
Restores a fallen ally to life.
Requires: Holy symbol and 5 holy water.
"""
import sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from engine.combat_techniques import CombatTechniqueEngine, TECHNIQUE_RESURRECTION

def handle_resurrection(player_state, phrase_input, world_map, current_tick,
                        action_queue, technique_manager=None, equipment_engine=None,
                        combat_engine=None):
    player_id = player_state.get("id", "player")
    if technique_manager and not technique_manager.has_technique(player_id, "resurrection"):
        return {"success": False, "message": "You do not know Resurrection.", "events": []}
    ...
    if combat_engine and hasattr(combat_engine, '_technique_engine'):
        return combat_engine._technique_engine.use_technique(
            actor_id=player_id, technique_id="resurrection", target_id=target_id,
