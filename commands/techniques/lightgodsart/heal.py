"""
Heal Command (Divine Technique)
Real command: cast heal <target>
Restores health to a target.
Requires: Holy symbol and 1 holy water.
"""
import sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from engine.combat_techniques import CombatTechniqueEngine, TECHNIQUE_HEAL

def handle_heal(player_state, phrase_input, world_map, current_tick,
                action_queue, technique_manager=None, equipment_engine=None,
                combat_engine=None):
    player_id = player_state.get("id", "player")
    if technique_manager and not technique_manager.has_technique(player_id, "heal"):
        return {"success": False, "message": "You do not know Heal.", "events": []}
    ...
    if combat_engine and hasattr(combat_engine, '_technique_engine'):
        return combat_engine._technique_engine.use_technique(
            actor_id=player_id, technique_id="heal", target_id=target_id,
