"""
Reality Tear Command (Arcane Technique)

Real command: cast reality tear <target>

Tears a rift in reality, dealing massive damage.
Requires: Arcane focus (staff/wand) and 5 runestones.
"""
...
from engine.combat_techniques import CombatTechniqueEngine, TECHNIQUE_REALITY_TEAR


def handle_reality_tear(player_state, phrase_input, world_map, current_tick,
                        action_queue, technique_manager=None, equipment_engine=None,
                        combat_engine=None):
    """Handle reality tear casting."""
    player_id = player_state.get("id", "player")

    if technique_manager and not technique_manager.has_technique(player_id, "reality_tear"):
        return {
            "success": False,
            "message": "You do not know the Reality Tear technique.",
            "events": []
        }
