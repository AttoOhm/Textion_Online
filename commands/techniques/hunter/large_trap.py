...
from engine.combat_techniques import CombatTechniqueEngine, TECHNIQUE_LARGE_TRAP

def handle_large_trap(player_state, phrase_input, world_map, current_tick,
                       action_queue, technique_manager=None, equipment_engine=None,
                       combat_engine=None):
    player_id = player_state.get("id", "player")
    if technique_manager and not technique_manager.has_technique(player_id, "large_trap"):
        return {"success": False, "message": "You do not know Large Trap.", "events": []}
    
    if combat_engine and hasattr(combat_engine, '_technique_engine'):
        return combat_engine._technique_engine.use_technique(
            actor_id=player_id, technique_id="large_trap", target_id=player_id,
