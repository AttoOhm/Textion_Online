...
from engine.combat_techniques import CombatTechniqueEngine, TECHNIQUE_DEATH_MARK

def handle_death_mark(player_state, phrase_input, world_map, current_tick,
                      action_queue, technique_manager=None, equipment_engine=None,
                      combat_engine=None):
    player_id = player_state.get("id", "player")
    if technique_manager and not technique_manager.has_technique(player_id, "death_mark"):
        return {"success": False, "message": "You do not know Death Mark.", "events": []}
    ...
    if combat_engine and hasattr(combat_engine, '_technique_engine'):
        return combat_engine._technique_engine.use_technique(
            actor_id=player_id, technique_id="death_mark", target_id=target_id,
