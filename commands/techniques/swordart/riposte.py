"""
Riposte Command (Sword Technique)

Real command: riposte <target>

A counter-attack that taunts the target.
...
def handle_riposte(player_state, target_id, world_map, current_tick, action_queue, technique_manager=None, equipment_engine=None, combat_engine=None):
    """Handle the 'riposte <target>' command."""
    # Check technique ownership
    if technique_manager:
        if not technique_manager.has_technique(player_state.get("id", "player"), "riposte"):
            return {
                "success": False,
                "message": "You do not know the Riposte technique.",
                "events": []
            }
    ...
    if equipment_engine:
        ...
        if not equipped:
            return {
                "success": False,
                "message": "You need a sword equipped to use Riposte.",
                "events": []
            }
    ...
    # Create Long Action for riposte
    ...
    if not action:
        return {
            "success": False,
            "message": "Failed to start Riposte.",
            "events": []
        }

    return {
        "success": True,
        "message": f"You prepare a counter-attack against {target_data.get('name', target_id)}...",
        "action_id": action["action_id"],
        "events": action_queue.get_events()
    }


def resolve_riposte(action, player_state, world_map, current_tick, combat_engine, technique_engine=None):
    """Resolve a completed riposte action."""
    ...
    if outcome in ("success", "exceptional_success", "partial_success"):
        ...
        message += f" Riposte taunts the target!"
        ...
        return {
            "outcome": outcome,
            "message": f"Your Riposte misses {target_name}!",
            "resolution": resolution,
        }
